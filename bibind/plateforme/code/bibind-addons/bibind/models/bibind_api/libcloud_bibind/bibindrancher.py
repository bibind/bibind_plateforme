...
class BibindRancherContainerDriver(ContainerDriver):
    """
    ...
    def __init__(self, key, secret, secure=True, host='localhost', port=443):
        ...
        super(BibindRancherContainerDriver, self).__init__(
            key=key,
            secret=secret,
    ...
    def ex_deploy_v2_stack(self, name, description=None, env_id=None, docker_compose=None,
                        environment=None, external_id=None,
                        rancher_compose=None, start=True):
        """
        Deploy a new stack.
        ...
        data = json.dumps(dict((k, v) for (k, v) in payload.items()
                               if v is not None))
        result = self.connection.request('%s/projects/%/stack/' %(
                                         self.baseuri, env_id), data=data,
                                         method='POST').object
    ...
    def ex_search_env(self):
        ...
        :rtype: ``dict``
        """
        result = self.connection.request("%s/projects" %
                                         (self.baseuri)).object
    ...
    def ex_destroy_v2_stack(self, env_id=None, stack_id=None):
        """
        Destroy a stack by ID
        ...
        :rtype: ``bool``
        """
        url = '%s/projects/%s/stacks/%s' % (self.baseuri, env_id, stack_id)
        
    
        result = self.connection.request('%s/projects/%s/stacks/%s' % (
                                         self.baseuri, env_id, stack_id),
                                         method='DELETE')
        return result.status in VALID_RESPONSE_CODES
